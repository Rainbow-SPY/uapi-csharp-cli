...
namespace UAPI.CliGenerated
{
    public static class Cli_WebParse_WebParse_ToMarkdown
    {
            ...
            Option<string> authenticationOption)
        {
            ...
            cmd_web_parse_web_to_markdown_async_1.SetAction(async parseResult =>
            {
                var url = parseResult.GetValue(opt_web_parse_web_to_markdown_async_1_url);
                var Authentication = parseResult.GetValue(authenticationOption);
                var result = await WebParse.PostWebToMarkdownAsync(url, Authentication);
            ...
            cmd_web_parse_web_to_markdown_result_2.SetAction(async parseResult =>
            {
                var taskId = parseResult.GetValue(opt_web_parse_web_to_markdown_result_2_taskId);
                var Authentication = parseResult.GetValue(authenticationOption);
                var result = await WebParse.GetWebToMarkdownResult(taskId, Authentication);
                CliOutput.WriteObject(result, parseResult.GetValue(outOption), parseResult.GetValue(appendOption));
            });
        }
    }
