...
namespace UAPI.CliGenerated
{
    public static class Cli_minecraft_GetServerStatus
    {
            ...
            Option<string> authenticationOption, Option<string> resultOption, Option<string> selectOption)
        {
            ...
            cmd_minecraft_server_status_1.SetAction(parseResult =>
            {
                var server = parseResult.GetValue(opt_minecraft_server_status_1_server);
                var Authentication = parseResult.GetValue(authenticationOption);
                var result = Minecraft.GetServerStatus(server, Authentication).GetAwaiter().GetResult();
            ...
            cmd_minecraft_server_status_by_server_2.SetAction(parseResult =>
            {
                var server = parseResult.GetValue(opt_minecraft_server_status_by_server_2_server);
                var result = Minecraft.GetServerStatus(server).GetAwaiter().GetResult();
                CliOutput.WriteObject(result, parseResult.GetValue(outputOption), parseResult.GetValue(appendOption),
                    parseResult.GetValue(resultOption), parseResult.GetValue(selectOption));
                return 0;
            });
            ...
            cmd_minecraft_server_status_by_server_3.SetAction(parseResult =>
            {
