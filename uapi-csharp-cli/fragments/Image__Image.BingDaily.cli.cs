...
namespace UAPI.CliGenerated
{
    public static class Cli_Image_Image_BingDaily
    {
            ...
            Option<string> authenticationOption)
        {
            ...
            cmd_image_bing_daily_meta_json_data_1.SetAction(async parseResult =>
            {
                ...
                var result = await Image.GetBingDailyMetaJsonData(date, resolutions, Authentication);
            ...
            cmd_image_bing_daily_meta_json_data_by_2_2.SetAction(async parseResult =>
            {
                ...
                var Authentication = parseResult.GetValue(authenticationOption);
                var result = await Image.GetBingDailyMetaJsonData(random, resolutions, Authentication);
                CliOutput.WriteObject(result, parseResult.GetValue(outOption), parseResult.GetValue(appendOption));
            });
            ...
            var opt_image_bing_daily_image_3_resolutions = new Option<Type.BingDailyType.Resolutions>("--resolutions")
            {
                Required = false, Description = "指定返回图像的分辨率, 默认4K, 可选 1080P",
                DefaultValueFactory = _ => Type.BingDailyType.Resolutions._4K
            };
            ...
            cmd_image_bing_daily_image_3.SetAction(async parseResult =>
            {
                ...
                var Authentication = parseResult.GetValue(authenticationOption);
                var result = await Image.GetBingDailyImage(date, resolutions, format, Authentication);
            ...
            cmd_image_bing_daily_image_by_2_4.SetAction(async parseResult =>
            {
                ...
                var Authentication = parseResult.GetValue(authenticationOption);
                var result = await Image.GetBingDailyImage(random, resolutions, format, Authentication);
                CliOutput.WriteBytes(result, parseResult.GetValue(outOption), parseResult.GetValue(appendOption));
            });
            ...
            var opt_image_bing_daily_history_5_resolution = new Option<Type.BingDailyType.Resolutions>("--resolution")
            {
            ...
            cmd_image_bing_daily_history_5.SetAction(async parseResult =>
            {
                ...
                var Authentication = parseResult.GetValue(authenticationOption);
                var result = await Image.GetBingDailyHistory(date, resolution, page, pageSize, Authentication);
                CliOutput.WriteObject(result, parseResult.GetValue(outOption), parseResult.GetValue(appendOption));
            });
        }
    }
